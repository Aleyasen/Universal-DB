...
function printToFile($loc, $v_out, $e_out, $vs, $h_VGV, $h_VVG, $h_VA, $h_L, $h_EG) {
    ...
    foreach ($v_l as $l) {
        $file = fopen($loc . $h_L[$l] . ".filter", "w") or die("unable to create " . $loc . $h_L[$l] . ".filter" . " !");
        foreach ($vs as $vg) {
            $v_id = $h_VGV[$vg];
            $l_id = $v_id % 100;
            if ($l_id == $l) {
    ...
    foreach ($e_l as $l) {
        $l1 = $h_L[$l[0]];
        $l2 = $h_L[$l[1]];
        $file = fopen($loc . $h_L[$l1] . "_" . $h_L[$l2] . ".filter", "w") or die("unable to create " . $loc . $h_L[$l1] . "_" . $h_L[$l2] . ".filter" . " !");
